 ...
 * <a href="http://goosebumps4all.net/34all/bb/showthread.php?tid=45">thread</a> 
 * 
...
public class GraphViewDistanceFilterVariationsInDMinorMagic extends JPanel {
    ...
    //search needs to be renewable for opened files
	...
	final ForceDirectedLayoutMagic fdlm = new ForceDirectedLayoutMagic(
		graph);
	...
	ForceSimulator fsim = ((ForceDirectedLayoutMagic) animate.get(0))
		.getForceSimulator();
	...
	searchBox = new Box(BoxLayout.Y_AXIS);
	searchBox.add(getSearchPanel(sq));
    ...
    /**
     * Have to be able to create 'new' search panel when a new file is loaded
     * @param sq
     * @return
     */
    private JSearchPanel getSearchPanel(SearchQueryBinding sq) {
	searchPanel = sq.createSearchPanel();
	searchPanel.setToolTipText("Find an item by searching for it by name");
	...
	//renew the searchBox on load of new graph
	if (searchBox != null) {
	    searchBox.remove(searchPanel);
	    // create a new query and search panel for the graph
	    SearchQueryBinding sq = new SearchQueryBinding((Table) m_vis
		    .getGroup(nodes), label, (SearchTupleSet) m_vis
		    .getGroup(Visualization.SEARCH_ITEMS));
	    searchBox.add(getSearchPanel(sq));
	...
	String datafile = "datasets/socialnet.xml";
	String label = "name";
    ...
    /**
     * A simple control maintaining a focus group which contains only the node
	...
	public void tupleSetChanged(TupleSet ts, Tuple[] add, Tuple[] rem) {
		...
		if (previousVisibleItems != null) {
		    Iterator iter = previousVisibleItems.iterator();
		    // reconstructimg the pre filtered state
		    while (iter.hasNext()) {
		...
		if (!lastTimeFiltered) {
		    // remembering the last unfiltered set of visible items
...
class ForceDirectedLayoutMagic extends Layout {
     ...
     *                instance.
     * @param enforceBounds
     *                indicates whether or not the layout should require that
     *                all node placements stay within the layout bounds.
     */
     ...
     *                animated fashion. In run-once mode, the layout will run
     *                for a set number of iterations when invoked. In animation
     *                mode, only one iteration of the layout is computed.
     */
    public ForceDirectedLayoutMagic(String group, boolean enforceBounds,
	    boolean runonce) {
	super(group);
     ...
     * @see prefuse.util.PrefuseLib#setX(prefuse.visual.VisualItem, prefuse.visual.VisualItem, double)
     * @see prefuse.util.PrefuseLib#setY(prefuse.visual.VisualItem, prefuse.visual.VisualItem, double)
     */
